...
int melody(int sel) {
  ...
  // MODE B-2 MELODY
    ...
    int b2size = sizeof(b2melody) / sizeof(int);
    for(int n = 0; n < b2size; n++) {
      int b2nduration = 1000 / b2tempo[n];
      tone(speakerPin, b2melody[n], b2nduration);
      int b2pause = b2nduration * 1.30;
      delay(b2pause);

      tone(speakerPin, 0, b2nduration);
    }
  }
  // MODE B-3 MELODY
  else if(sel == 7) {
    ...
    int b3size = sizeof(b3melody) / sizeof(int);
    for (int o = 0; o < b3size; o++) {
      int b3nduration = 1000 / b3tempo[o];
      tone(speakerPin, b3melody[o], b3nduration);
      int b3pause = b3nduration * 1.30;
      delay(b3pause);

      tone(speakerPin, 0, b3nduration);
    }
  }
}
